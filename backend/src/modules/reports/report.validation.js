...
function validDate(value, name) {
  ...
  if (text && !DateTime.fromISO(text).isValid)
    throw AppError.badRequest('VALIDATION_ERROR', `${name} must be a valid date`)
  return text
}
...
export function validateReportPeriod(query) {
  ...
  if (month && !DateTime.fromFormat(month, 'yyyy-MM').isValid)
    throw AppError.badRequest('VALIDATION_ERROR', 'month must be valid')
  return { date, month }
}
