...
function id(req, _res, next) {
  try {
    req.validatedId = validateInventoryProductId(req.params.productId)
    next()
  } catch (error) {
    next(error)
  }
