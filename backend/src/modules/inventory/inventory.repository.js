...
const PRODUCT_FIELDS = 'id, name, sku, barcode, category, unit, low_stock_threshold, is_active, deleted_at'
const MOVEMENT_FIELDS = 'id, product_id, movement_type, quantity_change, quantity_before, quantity_after, reference_type, reference_id, reason, created_by, created_at'
...
export class InventoryRepository {
  ...
  async list(shopId, filters, token) {
    ...
    let productsQuery = this.forAccessToken(token).from('products')
      .select(PRODUCT_FIELDS, { count: 'exact' }).eq('shop_id', shopId)
    ...
    const productsResult = await productsQuery.order('name').range(from, from + filters.pageSize - 1)
    ...
    const { data: balances, error } = await this.forAccessToken(token).from('inventory')
      .select('product_id, quantity, updated_at').in('product_id', products.map((product) => product.id))
    if (error) throw inventoryError(error)
    const balancesByProduct = new Map((balances ?? []).map((balance) => [balance.product_id, balance]))
    return {
      rows: products.map((product) => ({ product, balance: balancesByProduct.get(product.id) ?? null })),
      count: productsResult.count ?? 0
    }
  }

  async findProduct(shopId, productId, token, activeOnly) {
    let query = this.forAccessToken(token).from('products').select(PRODUCT_FIELDS)
  ...
  async findBalance(productId, token) {
    const { data, error } = await this.forAccessToken(token).from('inventory')
      .select('product_id, quantity, updated_at').eq('product_id', productId).maybeSingle()
    if (error) throw inventoryError(error)
    return data
  }
  ...
  async lowStock(shopId, filters, token) {
    const from = (filters.page - 1) * filters.pageSize
    const { data, error, count } = await this.forAccessToken(token).from('low_stock_alerts')
      .select('shop_id, product_id, name, sku, barcode, unit, quantity, low_stock_threshold, updated_at', {
        count: 'exact'
      }).eq('shop_id', shopId).order('name').range(from, from + filters.pageSize - 1)
    if (error) throw inventoryError(error)
  ...
  async movements(shopId, productId, filters, token) {
    const from = (filters.page - 1) * filters.pageSize
    const { data, error, count } = await this.forAccessToken(token).from('inventory_movements')
      .select(MOVEMENT_FIELDS, { count: 'exact' }).eq('shop_id', shopId).eq('product_id', productId)
      .order('created_at', { ascending: false }).range(from, from + filters.pageSize - 1)
    if (error) throw inventoryError(error)
    return { rows: data ?? [], count: count ?? 0 }
  }
