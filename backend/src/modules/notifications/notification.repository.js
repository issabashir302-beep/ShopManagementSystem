...
function failure() {
  return new AppError(500, 'NOTIFICATION_QUERY_FAILED', 'Unable to process monthly report delivery')
}
export class NotificationRepository {
  ...
  async ownerTarget(shopId) {
    const { data, error } = await this.adminClient
      .from('shops')
      .select('id, name, currency, owner_id, users!shops_owner_id_fkey(email)')
      .eq('id', shopId)
      .is('deleted_at', null)
      .maybeSingle()
  ...
  async report(shopId, start, end) {
    const { data, error } = await this.adminClient.rpc('get_shop_report', {
      p_shop_id: shopId,
      p_start: start,
      p_end: end
    })
  ...
  async claim(shopId, month, requestId) {
    const { data, error } = await this.adminClient.rpc('claim_report_delivery', {
      p_shop_id: shopId,
      p_report_month: `${month}-01`,
