...
export class NotificationService {
  constructor({ repository, reportService, emailClient, fromEmail, timezone, logger }) {
    Object.assign(this, { repository, reportService, emailClient, fromEmail, timezone, logger })
  }
  async sendManual(auth, month, requestId) {
    ...
    if (period.key > DateTime.now().setZone(this.timezone).toFormat('yyyy-MM')) {
      throw AppError.badRequest('VALIDATION_ERROR', 'A future reporting month cannot be emailed')
    }
  ...
  async deliver(shopId, period, requestId = crypto.randomUUID()) {
    ...
    if (!claim) {
      this.logger.info('monthly_report_duplicate_skipped', {
        requestId,
        shopId,
    ...
    try {
      const [target, metrics] = await Promise.all([
        this.repository.ownerTarget(shopId),
        this.repository.report(shopId, period.start, period.end)
      ])
      if (!target)
        throw new AppError(
          422,
          'OWNER_EMAIL_MISSING',
          'The shop owner does not have a deliverable email address'
        )
      const report = {
        shop: { id: shopId, name: target.shop.name, currency: target.shop.currency },
        period: { ...period, timezone: this.timezone },
        ...metrics
      }
      const content = monthlyReportEmail(report)
      const result = await this.emailClient.send(
        { from: this.fromEmail, to: [target.email], ...content },
        `monthly-report/${shopId}/${period.key}`
      )
      ...
      this.logger.info('monthly_report_email_sent', {
        requestId,
        shopId,
        reportMonth: period.key,
      ...
      try {
        await this.repository.failed(claim.id, error.message)
      } catch (stateError) {
        this.logger.error('monthly_report_state_update_failed', {
          requestId,
          shopId,
          reportMonth: period.key,
          error: stateError
      ...
      this.logger.error('monthly_report_email_failed', {
        requestId,
        shopId,
        reportMonth: period.key,
        error
  ...
  async runMonthly(now = DateTime.now(), requestId = crypto.randomUUID()) {
    ...
    for (const shop of shops) {
      try {
        results.push(await this.deliver(shop.id, period, requestId))
      } catch {
        results.push({ sent: false, failed: true, shopId: shop.id })
      }
    }
