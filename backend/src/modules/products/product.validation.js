...
function productFields(body, creation) {
  ...
  const result = {}
  if (creation || 'name' in body)
    result.name = stringField(body.name, 'name', { required: true, min: 1, max: 160 })
  if ('sku' in body)
    result.sku = stringField(body.sku, 'sku', { max: 80, pattern: /^[A-Za-z0-9._/-]+$/ })
  if ('barcode' in body)
    result.barcode = stringField(body.barcode, 'barcode', {
      max: 100,
      pattern: /^[A-Za-z0-9._/-]+$/
    })
  if ('category' in body) result.category = stringField(body.category, 'category', { max: 100 })
  if (creation || 'unit' in body)
    result.unit = stringField(body.unit, 'unit', { required: true, min: 1, max: 40 })
  if ('buyingPrice' in body)
    result.buying_price = decimalField(body.buyingPrice, 'buyingPrice', { scale: 2 })
  if ('sellingPrice' in body)
    result.selling_price = decimalField(body.sellingPrice, 'sellingPrice', { scale: 2 })
  if ('lowStockThreshold' in body) {
    result.low_stock_threshold = decimalField(body.lowStockThreshold, 'lowStockThreshold', {
      scale: 3
    })
  }
  if (creation) {
  ...
  return {
    ...
    status:
      query.status === undefined
        ? 'active'
        : stringField(query.status, 'status', {
            required: true,
