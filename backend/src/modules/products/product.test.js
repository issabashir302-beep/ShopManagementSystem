...
function fixture({ role = 'owner', product = row, listRows = [row], error } = {}) {
  ...
  const productRepository = {
    ...
    async list(shopId, filters) {
      ...
      return { rows: listRows, count: listRows.length, filters }
    },
    async findById(shopId) {
      assert.equal(shopId, 'shop-1')
      return product
    },
    async update(shopId, id, changes) {
      updated = { shopId, id, changes }
      return product
    },
...
describe('ProductService', () => {
  ...
  it('rejects shopkeeper catalog writes', async () => {
    ...
    await assert.rejects(() => value.service.create(auth, { name: 'X' }, 'req'), {
      code: 'FORBIDDEN'
    })
    await assert.rejects(() => value.service.update(auth, PRODUCT_ID, { name: 'X' }, 'req'), {
      code: 'FORBIDDEN'
    })
  ...
  it('allows owner and shopkeeper to list only derived-shop active products', async () => {
    for (const role of ['owner', 'shopkeeper']) {
      const result = await fixture({ role }).service.list(auth, {
        status: 'active',
        page: 1,
        pageSize: 20
      })
  ...
  it('prevents shopkeepers from requesting archived catalog data', async () => {
    await assert.rejects(
      () =>
        fixture({ role: 'shopkeeper' }).service.list(auth, {
          status: 'all',
          page: 1,
          pageSize: 20
        }),
  ...
  it('preserves duplicate SKU/barcode conflict', async () => {
    const error = AppError.conflict(
      ...
      'SKU or barcode already exists in this shop'
    )
...
describe('products API validation', () => {
  ...
  it('passes validated name/SKU/barcode search and pagination', async () => {
    ...
    const result = await server.request(
      '/api/v1/products?search=CC500&category=Drinks&page=2&pageSize=10',
      {
        token: 'valid-token'
      }
    )
