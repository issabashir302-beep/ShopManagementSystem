...
export function validateShopkeeperUpdate(body) {
  ...
  if (Object.keys(body).length === 0) {
    throw AppError.badRequest(
      'VALIDATION_ERROR',
      'At least one shopkeeper profile field is required'
    )
  }
  const changes = {}
  if ('fullName' in body)
    changes.full_name = stringField(body.fullName, 'fullName', { required: true, min: 2, max: 100 })
  if ('phone' in body)
    changes.phone = stringField(body.phone, 'phone', { max: 30, pattern: /^[+0-9() .-]+$/ })
  if ('username' in body)
    changes.username = stringField(body.username, 'username', {
      min: 3,
      max: 50,
...
export function validateStatusUpdate(body) {
  ...
  if (typeof body.isActive !== 'boolean') {
    throw AppError.badRequest('VALIDATION_ERROR', 'isActive must be a boolean', {
      fields: ['isActive']
    })
  }
