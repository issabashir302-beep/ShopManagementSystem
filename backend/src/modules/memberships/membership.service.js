...
export class MembershipService {
  ...
  async create(auth, input, requestId) {
    ...
    try {
      ...
      const membership = await this.membershipRepository.createMembership(shop.id, authUser.id, auth.token)
      this.logger.info('shopkeeper_created', { requestId, shopId: shop.id, shopkeeperId: authUser.id })
  ...
  async list(auth) {
    ...
    const profiles = await this.membershipRepository.listProfiles(memberships.map((item) => item.user_id), auth.token)
  ...
  async requireOwnedShopkeeper(auth, shopkeeperId) {
    const shop = await this.requireOwnerShop(auth)
    const membership = await this.membershipRepository.findMembership(shop.id, shopkeeperId, auth.token)
  ...
  async update(auth, shopkeeperId, changes, requestId) {
    const { shop, membership, profile: oldProfile } = await this.requireOwnedShopkeeper(auth, shopkeeperId)
  ...
  async updateStatus(auth, shopkeeperId, isActive, requestId) {
    ...
    const updated = await this.membershipRepository.updateStatus(membership.id, isActive, auth.token)
    ...
    this.logger.info(isActive ? 'shopkeeper_activated' : 'shopkeeper_deactivated', {
      requestId, shopId: shop.id, shopkeeperId
    })
  ...
  async resetPassword(auth, shopkeeperId, requestId) {
    ...
    if (!profile.email) throw AppError.unprocessable('SHOPKEEPER_EMAIL_MISSING', 'Shopkeeper has no reset email')
    await this.membershipRepository.sendPasswordReset(profile.email)
    this.logger.info('shopkeeper_password_reset_requested', { requestId, shopId: shop.id, shopkeeperId })
