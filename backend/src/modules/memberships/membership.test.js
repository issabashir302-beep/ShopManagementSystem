...
function fixture({
  ...
  const repository = {
    ...
    async listMemberships() {
      return ownedMembership ? [ownedMembership] : []
    },
    async listProfiles() {
      return ownedMembership ? [profile] : []
    },
    async findMembership() {
      return ownedMembership
    },
    async updateProfileAdmin(_id, changes) {
      return { ...profile, ...changes }
    },
    async updateStatus(_id, isActive) {
      status = isActive
      return { ...membership, is_active: isActive }
    },
...
describe('MembershipService', () => {
  it('creates a shopkeeper linked to the owner shop with fixed role', async () => {
    ...
    const result = await service.create(
      OWNER,
      { email: profile.email, password: 'Password123' },
      'req'
    )
  ...
  it('compensates Auth/profile creation when membership setup fails', async () => {
    ...
    await assert.rejects(() =>
      fixtureValue.service.create(OWNER, { email: profile.email, password: 'Password123' }, 'req')
    )
