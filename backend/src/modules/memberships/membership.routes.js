...
import {
  validateShopkeeperCreate,
  validateShopkeeperId,
  validateShopkeeperUpdate,
...
function id(req, _res, next) {
  try {
    req.validatedId = validateShopkeeperId(req.params.shopkeeperId)
    next()
  } catch (error) {
    next(error)
  }
