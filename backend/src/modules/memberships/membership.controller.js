...
export function createMembershipController(service) {
  return {
    ...
    status: async (req, res) => sendSuccess(res, await service.updateStatus(
      req.auth, req.validatedId, req.validated.is_active, req.requestId
    )),
    resetPassword: async (req, res) => sendSuccess(res, await service.resetPassword(req.auth, req.validatedId, req.requestId))
  }
}
