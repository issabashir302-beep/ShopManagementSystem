...
describe('one-shop database invariant', () => {
  it('enforces one active shop and one active membership per user', async () => {
    const sql = await readFile(
      new URL('../../../../supabase/database.sql', import.meta.url),
      'utf8'
    )
    assert.match(sql, /shops_one_active_per_owner_uq/)
    assert.match(sql, /shop_memberships_one_active_shop_per_user_uq/)
  })
})
...
function makeService({ role = 'owner', owned = [], memberships = [], shop = shopRow } = {}) {
  ...
  return {
    service: new ShopService({
      userRepository: {
        async findById() {
          return { id: auth.userId, user_role: role }
        }
      },
      shopRepository: {
        async findOwnedShops() {
          return owned
        },
...
describe('ShopService', () => {
  ...
  it('rejects a second owner shop', async () => {
    ...
    await assert.rejects(() => service.createShop(auth, { name: 'X' }), {
...
describe('shops API validation and authentication', () => {
  ...
  it('requires authentication', async () => {
    ...
    assert.equal(
      (await server.request('/api/v1/shops', { method: 'POST', body: { name: 'X' } })).response
        .status,
      401
    )
  })
  ...
  for (const field of ['ownerId', 'owner_id', 'id', 'deletedAt', 'deleted_at', 'createdAt']) {
    it(`rejects protected shop field ${field}`, async () => {
      ...
      const result = await server.request('/api/v1/shops', {
        method: 'POST',
        token: 'valid-token',
        body: { name: 'Mini Mart', [field]: 'malicious' }
      })
  ...
  it('accepts safe shop updates and rejects protected updates', async () => {
    ...
    const valid = await server.request('/api/v1/shops/me', {
      method: 'PATCH',
      token: 'valid-token',
      body: { name: 'New Shop', currency: 'kes' }
    })
