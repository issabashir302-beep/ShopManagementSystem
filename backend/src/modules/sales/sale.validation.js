...
export function validateCheckout(body) {
  ...
  const method = stringField(body.payment.method, 'payment.method', {
    required: true
  }).toLowerCase()
  ...
  if (
    method === 'card' &&
    (body.payment.providerReference !== undefined || body.payment.externalReference !== undefined)
  ) {
    throw AppError.badRequest(
      'VALIDATION_ERROR',
      'Card provider references are assigned by the backend'
    )
  }
  ...
  const items = body.items.map((item, index) => {
    ...
    if (seen.has(productId))
      throw AppError.badRequest('DUPLICATE_PRODUCT', 'Duplicate product IDs are not allowed')
    seen.add(productId)
    return {
  ...
  return {
    ...
    payment: {
      method,
      providerReference: stringField(body.payment.providerReference, 'payment.providerReference', {
        max: 200
      }),
      externalReference: stringField(body.payment.externalReference, 'payment.externalReference', {
        max: 200
      })
    },
    items
...
export function validateSaleList(query) {
  ...
  if (status && !STATUSES.has(status))
    throw AppError.badRequest('VALIDATION_ERROR', 'Invalid sale status')
  ...
  if (dateFrom && dateTo && dateFrom > dateTo)
    throw AppError.badRequest('VALIDATION_ERROR', 'dateFrom must not be after dateTo')
  return {
    ...
    dateFrom,
    dateTo,
    soldBy: query.soldBy === undefined ? null : uuidField(query.soldBy, 'soldBy'),
    status
  }
}
