...
function fixture({
  context = { sale, payment, currency: 'KES' },
  stripeIntent = intent,
  stripeError,
  event,
  eventResult = { duplicate: false }
} = {}) {
  let created,
    attached,
    processed,
  ...
  const service = new PaymentService({
    paymentRepository: {
      async findStripeContext(shopId) {
        assert.equal(shopId, 'shop-1')
        return context
      },
      async attachStripeIntent(id, reference) {
        attached = { id, reference }
        return payment
      },
    ...
    stripeGateway: {
      async createPaymentIntent(input, key) {
...
describe('Stripe card PaymentIntent creation', () => {
  it('uses the authoritative database amount and stable Stripe idempotency key', async () => {
    const value = fixture()
    const result = await value.service.createStripeIntent(
      { token: 't' },
      { saleId: SALE_ID },
      'req'
    )
    ...
    assert.equal(result.clientSecret, 'pi_test_secret')
    assert.equal(result.status, 'pending')
  })
  it('reuses an attached PaymentIntent instead of creating another', async () => {
    const value = fixture({
      context: { sale, payment: { ...payment, provider_reference: 'pi_test' }, currency: 'KES' }
    })
    await value.service.createStripeIntent({ token: 't' }, { saleId: SALE_ID }, 'req')
    assert.equal(value.created(), undefined)
    assert.equal(value.retrieved(), 1)
  })
  it('returns safe not-found for invalid or cross-shop sale', async () => {
    await assert.rejects(
      () =>
        fixture({ context: null }).service.createStripeIntent(
          { token: 't' },
          { saleId: SALE_ID },
          'req'
        ),
  ...
  it('rejects an already-paid sale', async () => {
    ...
    await assert.rejects(
      () =>
        fixture({ context: paid }).service.createStripeIntent(
          { token: 't' },
          { saleId: SALE_ID },
          'req'
        ),
  ...
  it('maps Stripe API failures without leaking provider details', async () => {
    ...
    await assert.rejects(
      () =>
        fixture({ stripeError: error }).service.createStripeIntent(
          { token: 't' },
          { saleId: SALE_ID },
          'req'
        ),
  ...
  it('rejects inconsistent persisted payment amount', async () => {
    ...
    await assert.rejects(
      () =>
        fixture({ context }).service.createStripeIntent({ token: 't' }, { saleId: SALE_ID }, 'req'),
...
describe('Stripe webhook handling', () => {
  ...
  it('processes a signature-verified success through the trusted RPC', async () => {
    ...
    assert.deepEqual(value.processed(), {
      eventId: 'evt_1',
  ...
  it('processes payment failure without accepting browser status', async () => {
    const value = fixture({
      event: { ...succeeded, id: 'evt_2', type: 'payment_intent.payment_failed' }
    })
  ...
  it('reports duplicate webhook delivery safely', async () => {
    ...
    }).service.handleStripeWebhook(Buffer.from('{}'), 'signature', 'req')
    assert.equal(result.duplicate, true)
  })
  it('rejects missing or invalid webhook signatures', async () => {
    await assert.rejects(
      () =>
        fixture({ event: succeeded }).service.handleStripeWebhook(Buffer.from('{}'), null, 'req'),
    ...
    await assert.rejects(
      () =>
        fixture({ stripeError: invalid }).service.handleStripeWebhook(
          Buffer.from('{}'),
          'bad',
          'req'
        ),
...
describe('Payments API security', () => {
  ...
  it('requires authentication for PaymentIntent creation', async () => {
    ...
    assert.equal(
      (
        await server.request('/api/v1/payments/stripe/create-intent', {
          method: 'POST',
          body: { saleId: SALE_ID }
        })
      ).response.status,
  ...
  it('accepts only saleId and never amount or status', async () => {
    server = await startTestServer()
    for (const body of [
      { saleId: SALE_ID, amount: 1 },
      { saleId: SALE_ID, status: 'completed' }
    ]) {
      assert.equal(
        (
          await server.request('/api/v1/payments/stripe/create-intent', {
  ...
  it('creates an intent for an authenticated request', async () => {
    ...
    server = await startTestServer({
      paymentService: {
        ...
        async createStripeIntent(_auth, input) {
          received = input
          return { clientSecret: 'secret', status: 'pending' }
        }
      }
    })
    const result = await server.request('/api/v1/payments/stripe/create-intent', {
      method: 'POST',
      token: 'valid-token',
      body: { saleId: SALE_ID }
    })
    assert.equal(result.response.status, 201)
    assert.deepEqual(received, { saleId: SALE_ID })
    ...
    server = await startTestServer({
      paymentService: {
        ...
        async handleStripeWebhook(body, signature) {
          received = { isBuffer: Buffer.isBuffer(body), signature }
          return { received: true }
        }
      }
    })
    const result = await server.request('/api/v1/payments/stripe/webhook', {
      method: 'POST',
      body: { id: 'evt' },
      headers: { 'stripe-signature': 'sig' }
    })
    assert.equal(result.response.status, 200)
    assert.deepEqual(received, { isBuffer: true, signature: 'sig' })
  })
  it('keeps arbitrary payment status mutation unavailable', async () => {
    server = await startTestServer()
    assert.equal(
      (
        await server.request(`/api/v1/payments/${PAYMENT_ID}`, {
          method: 'PATCH',
          token: 'valid-token',
