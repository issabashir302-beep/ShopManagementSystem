...
export function toStripeAmount(decimal, currency) {
  ...
  const scale = ZERO_DECIMAL.has(normalizedCurrency)
    ? 0
    : THREE_DECIMAL.has(normalizedCurrency)
      ? 3
      : 2
  ...
  if (fraction.length > scale && /[1-9]/.test(fraction.slice(scale))) {
    throw new AppError(
      ...
      'Stored amount cannot be represented in the payment currency'
    )
  }
  const minor =
    BigInt(match[1]) * 10n ** BigInt(scale) +
    BigInt((fraction.slice(0, scale) || '').padEnd(scale, '0') || '0')
  if (minor <= 0n || minor > 99_999_999n)
    throw AppError.unprocessable(
      'INVALID_PAYMENT_AMOUNT',
      'Payment amount is outside Stripe limits'
    )
  return Number(minor)
}
