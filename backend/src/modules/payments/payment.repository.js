...
function paymentError(error) {
  ...
  if (message.includes('Card payment was not found'))
    return AppError.notFound('PAYMENT_NOT_FOUND', 'Payment was not found')
...
export class PaymentRepository {
  ...
  async findStripeContext(shopId, saleId, token) {
    ...
    const [sale, shop] = await Promise.all([
      client
        .from('sales')
        .select('id, shop_id, status, payment_status, total_amount')
        .eq('shop_id', shopId)
        .eq('id', saleId)
        .maybeSingle(),
    ...
    if (payment.error) throw paymentError(payment.error)
    return payment.data
      ? { sale: sale.data, payment: payment.data, currency: shop.data.currency }
      : null
  }
