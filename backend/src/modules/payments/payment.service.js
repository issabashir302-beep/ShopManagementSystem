...
export class PaymentService {
  ...
  async listForSale(auth, saleId, filters) {
    ...
    return {
      items: result.rows.map(publicPayment),
      pagination: {
        page: filters.page,
  ...
  async createStripeIntent(auth, input, requestId) {
    const context = await this.shopService.getCurrentShopContext(auth)
    const value = await this.paymentRepository.findStripeContext(
      context.shop.id,
      input.saleId,
      auth.token
    )
    if (!value) throw AppError.notFound('SALE_NOT_FOUND', 'Eligible card sale was not found')
    if (value.sale.status !== 'completed')
      throw AppError.conflict('INVALID_SALE_STATUS', 'Sale is not eligible for payment')
    ...
    if (!['pending', 'failed'].includes(value.payment.status)) {
      throw AppError.conflict(
        ...
        'Payment is not eligible for card processing'
      )
    }
    if (value.payment.status === 'failed' && !value.payment.provider_reference) {
      throw AppError.conflict(
        'INVALID_PAYMENT_STATUS',
        'Failed card payment has no reusable Stripe intent'
      )
    }
    ...
    if (toStripeAmount(value.payment.amount, value.currency) !== amount) {
      throw new AppError(
        ...
        'Stored payment amount does not match the sale'
      )
    }
    let intent
    ...
    if (
      ...
      intent.currency.toLowerCase() !== value.currency.toLowerCase()
    ) {
      throw new AppError(
        500,
        'STRIPE_INTENT_MISMATCH',
        'Stored Stripe intent does not match the sale'
      )
    }
    this.logger.info('stripe_intent_ready', {
      requestId,
  ...
  async handleStripeWebhook(rawBody, signature, requestId) {
    if (!signature)
      throw AppError.badRequest('INVALID_STRIPE_SIGNATURE', 'Stripe-Signature header is required')
    ...
    const result = await this.paymentRepository.processStripeEvent({
      eventId: event.id,
