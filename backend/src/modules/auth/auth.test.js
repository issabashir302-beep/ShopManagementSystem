...
describe('authentication API', () => {
  ...
  it('accepts valid owner signup without forwarding a role', async () => {
    ...
    server = await startTestServer({
      authService: {
        async signup(input) { received = input; return { profile: { userRole: 'owner' } } },
        async verifyAccessToken() { return { id: 'user-1', email: 'x@example.com' } }
      }
    })
  ...
  for (const [name, body] of [
    ...
    ['caller role', { email: 'a@example.com', password: 'password1', fullName: 'Shop Owner', role: 'owner' }],
    ['caller userRole', { email: 'a@example.com', password: 'password1', fullName: 'Shop Owner', userRole: 'shopkeeper' }]
  ]) {
  ...
  it('rejects missing, malformed, and invalid authorization', async () => {
    ...
    const malformed = await server.request('/api/v1/auth/session', { headers: { authorization: 'Basic abc' } })
  ...
  it('requires authentication for logout and invokes server-side logout', async () => {
    ...
    server = await startTestServer({
      authService: {
        async verifyAccessToken(token) { return { id: 'user-1', email: 'a@example.com', token } },
        async logout(token) { loggedOutToken = token; return { message: 'Session revoked' } }
      }
    })
    const missing = await server.request('/api/v1/auth/logout', { method: 'POST' })
    const valid = await server.request('/api/v1/auth/logout', { method: 'POST', token: 'valid-token' })
    assert.equal(missing.response.status, 401)
...
describe('AuthService signup consistency', () => {
  function service({ profile, profileError, cleanupError } = {}) {
    ...
    const adminClient = {
      ...
      auth: { admin: { async deleteUser() { cleanupCalls += 1; return { error: cleanupError } } } }
    }
    return {
      instance: new AuthService({
        publicClient: { auth: { async signUp() {
          return { data: { user: { id: 'auth-1', email: 'a@example.com' }, session: null }, error: null }
        } } },
        adminClient,
        forAccessToken() {},
        logger: { error() {} }
  ...
  it('accepts the owner profile created by the database trigger', async () => {
    ...
    const result = await instance.signup({ email: 'a@example.com', password: 'password1', fullName: 'Owner' })
  ...
  it('compensates when profile creation fails', async () => {
    ...
    await assert.rejects(() => instance.signup({ email: 'a@example.com', password: 'password1', fullName: 'Owner' }), {
  ...
  it('returns only the selected session fields on login', async () => {
    const instance = new AuthService({
      publicClient: { auth: { async signInWithPassword() {
        return {
          data: {
            user: { id: 'auth-1', email: 'a@example.com', user_metadata: { secret: 'not-returned' } },
            session: {
              access_token: 'access', refresh_token: 'refresh', expires_at: 123,
              token_type: 'bearer', provider_token: 'not-returned'
            }
          },
          error: null
        }
      } } },
      adminClient: {}, forAccessToken() {}, logger: { error() {} }
    })
  ...
  it('uses one safe error for incorrect password or unknown user', async () => {
    const instance = new AuthService({
      publicClient: { auth: { async signInWithPassword() { return { data: {}, error: { message: 'internal auth detail' } } } } },
      adminClient: {}, forAccessToken() {}, logger: { error() {} }
    })
    await assert.rejects(() => instance.login({ email: 'unknown@example.com', password: 'password1' }), {
