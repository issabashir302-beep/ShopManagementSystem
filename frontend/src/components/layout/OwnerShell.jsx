...
export function OwnerShell() {
  ...
  return <div className="min-h-screen bg-[#f6f7f5] lg:grid lg:grid-cols-[224px_1fr]">
    {open && <button className="fixed inset-0 z-30 bg-black/35 lg:hidden" onClick={() => setOpen(false)} aria-label="Close navigation" />}
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-56 flex-col border-r bg-white transition-transform lg:static lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <nav className="flex-1 space-y-1 p-3">{links.map(([to, label, Icon, end]) => <NavLink key={to} to={to} end={end} onClick={() => setOpen(false)} className={({ isActive }) => `flex min-h-10 items-center gap-3 rounded-lg px-3 text-sm font-semibold ${isActive ? 'bg-brand-50 text-brand-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}><Icon size={18} />{label}</NavLink>)}</nav>
    ...
    <div className="min-w-0"><header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b bg-white/95 px-4 backdrop-blur lg:px-7"><button className="lg:hidden" onClick={() => setOpen(true)} aria-label="Open navigation"><Menu /></button><span className="hidden text-sm text-gray-500 sm:block">Owner workspace</span><NavLink to="/pos" className="rounded-lg border px-3 py-2 text-xs font-bold text-brand-700 hover:bg-brand-50">Open POS</NavLink></header><main className="mx-auto max-w-[1440px] p-4 sm:p-6 lg:p-8"><Outlet /></main></div>
  </div>
}
