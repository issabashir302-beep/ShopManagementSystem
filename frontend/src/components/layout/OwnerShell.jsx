...
import { ChartNoAxesCombined, CreditCard, Grid2X2, LogOut, Menu, PackageSearch, ReceiptText, Settings, ShoppingCart, UserRound, UsersRound, X } from 'lucide-react'
...
export function OwnerShell() {
  ...
  return <div className="min-h-screen min-h-[100dvh] bg-[#f6f7f5] lg:grid lg:grid-cols-[224px_minmax(0,1fr)]">
    ...
    <div className="min-w-0"><header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b bg-white/95 px-4 backdrop-blur lg:px-7"><button className="grid size-11 place-items-center rounded-lg lg:hidden" onClick={() => setOpen(true)} aria-label="Open navigation"><Menu /></button><span className="hidden text-sm text-gray-500 sm:block">Owner workspace</span><NavLink to="/pos" className="inline-flex min-h-10 items-center gap-2 rounded-lg border px-3 py-2 text-xs font-bold text-brand-700 hover:bg-brand-50"><ShoppingCart size={16} /><span className="hidden min-[360px]:inline">Open </span>POS</NavLink></header><main className="mx-auto w-full max-w-[1440px] p-3 min-[380px]:p-4 sm:p-6 lg:p-8"><Outlet /></main></div>
  </div>
}
