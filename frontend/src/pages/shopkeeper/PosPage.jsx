...
import { Minus, Plus, ScanLine, Search, ShoppingCart, Trash2, X } from 'lucide-react'
...
export function PosPage() {
  ...
  const products = useMemo(() => {
    ...
    return (productsQuery.data?.items || []).map((product) => ({ ...product, stock: Number(balances.get(product.id)?.quantity || 0), isLowStock: balances.get(product.id)?.isLowStock }))
  ...
  return <main className="grid min-h-[calc(100vh-64px)] lg:grid-cols-[1fr_390px]">
    ...
    {card && <CardPaymentModal {...card} onClose={() => setCard(null)} onConfirmed={async (saleId) => { const settled = await pollPaymentStatus(saleId, shopwiseApi.sales.get); const detail = settled?.detail || await shopwiseApi.sales.get(saleId); setResult({ sale: detail.sale, items: detail.items, payment: detail.payments?.[0] }); setCard(null) }} />}
  </main>
}
