...
import { Archive, Boxes, Edit3, History, PackagePlus, Plus, Search, SlidersHorizontal } from 'lucide-react'
...
function ProductList({ items, edit, adjust, history, archive }) { return <section className="card overflow-hidden rounded-xl">
  <div className="divide-y lg:hidden">{items.map((item) => <article className="p-4" key={item.id}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><strong className="block truncate">{item.name}</strong><span className="mt-1 block truncate text-xs text-gray-500">{item.sku || item.category || 'Uncategorised'} · {item.unit}</span></div><StockBadge item={item} /></div><div className="mt-4 grid grid-cols-3 gap-2 rounded-lg bg-gray-50 p-3 text-sm"><Metric label="Selling" value={money(item.sellingPrice)} /><Metric label="Buying" value={money(item.buyingPrice)} /><Metric label="Quantity" value={`${item.quantity} ${item.unit}`} /></div><div className="mt-3 grid grid-cols-2 gap-2 min-[480px]:grid-cols-4"><Button size="sm" variant="secondary" onClick={() => edit(item)}><Edit3 size={15} />Edit</Button><Button size="sm" variant="secondary" disabled={!item.isActive} onClick={() => adjust(item)}><Plus size={15} />Stock</Button><Button size="sm" variant="ghost" onClick={() => history(item)}><History size={15} />History</Button>{item.isActive && <Button size="sm" variant="ghost" onClick={() => archive.mutate(item.id)}><Archive size={15} />Archive</Button>}</div></article>)}</div>
  <div className="table-wrap hidden lg:block"><table className="data-table"><thead><tr><th>Product</th><th>Prices</th><th>Quantity</th><th>Stock</th><th>Updated</th><th>Actions</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td><strong>{item.name}</strong><small className="block text-gray-500">{item.sku || 'No SKU'} · {item.category || 'Uncategorised'} · {item.unit}</small></td><td><strong>{money(item.sellingPrice)}</strong><small className="block text-gray-500">Cost {money(item.buyingPrice)}</small></td><td className="font-bold">{item.quantity} <small className="font-normal text-gray-500">{item.unit}</small></td><td><StockBadge item={item} /></td><td>{dateTime(item.updatedAt)}</td><td><div className="flex justify-end gap-1"><Button size="sm" variant="ghost" onClick={() => edit(item)}><Edit3 size={15} /><span className="sr-only">Edit</span></Button><Button size="sm" variant="ghost" disabled={!item.isActive} onClick={() => adjust(item)}><Plus size={15} />Stock</Button><Button size="sm" variant="ghost" onClick={() => history(item)}><History size={15} /><span className="sr-only">History</span></Button>{item.isActive && <Button size="sm" variant="ghost" onClick={() => archive.mutate(item.id)}><Archive size={15} /><span className="sr-only">Archive</span></Button>}</div></td></tr>)}</tbody></table></div>
