...
export const shopwiseApi = {
  ...
  mpesa: { availability: () => api('/payment-methods'), settings: () => api('/mpesa/settings'), save: (body) => api('/mpesa/settings', { method: 'PUT', body }), verify: () => api('/mpesa/verify', { method: 'POST' }), initiate: (body) => api('/mpesa/stk-push', { method: 'POST', body }), attempt: (id) => api(`/mpesa/attempts/${id}`), confirmManual: (body) => api('/mpesa/manual-confirmation', { method: 'POST', body }) },
  expenses: {
